...
try:
    from .local_server_client import LocalServerClient
except ImportError:
    pass
...
def get_llm_client(provider: str, config_manager, **kwargs):
    ...
    :param provider: Name of the LLM provider ("gpt", "gemini", "local", "local_server")
    ...
    elif provider.lower() == "local_server":
        if LocalServerClient is None:
            raise ValueError(
                "LocalServerClient not available. Please install the 'openai' package."
            )
        return LocalServerClient(config_manager=config_manager, **kwargs)
    else:
